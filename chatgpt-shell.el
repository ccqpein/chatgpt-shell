  ...
  (if-let* ((last-label (chatgpt-shell--model-label))
            (width (let ((width)
                     ...
                     (mapc (lambda (model)
                             (unless width
                               (setq width 0))
                             (when-let ((provider-width (length (map-elt model :provider)))
                                        (longer (> provider-width width)))
                               (setq width provider-width)))
                           chatgpt-shell-models)
                     width))
            (short-width (let (short-width)
                           (mapc (lambda (model)
                                   (unless short-width
                                     (setq short-width  0))
                                   (when-let ((short-version-width (length (map-elt model :short-version)))
                                              (longer (> short-version-width short-width)))
                                     (setq short-width short-version-width)))
                                 chatgpt-shell-models)
                           short-width))
            (models (seq-map (lambda (model)
                               (format (format "%%-%ds   %%-%ds   %%s" width short-width)
                                       (map-elt model :provider)
                                       (if-let ((short-version (map-elt model :short-version))
                                                short-version)
                                           short-version
                                         "")
                                       (map-elt model :version)))
...
(cl-defun chatgpt-shell--resolved-model (&key versioned)
  ...
  (seq-find (lambda (model)
              (or (equal (map-elt model :version)
                         (or versioned (chatgpt-shell-model-version)))
                  (equal (map-elt model :short-version)
                         (or versioned (chatgpt-shell-model-version)))))
            chatgpt-shell-models))
